...
互联网热度是创意候选的独立评分维度，但不能替代商品事实、目标平台适配和可执行性判断。

数据优先级：

```text
目标平台 + 目标市场的热销榜、搜索结果或后台信号
-> 目标市场 Google Trends / 本地趋势工具
-> 目标市场 TikTok、Instagram、YouTube、Pinterest、Reddit 等内容信号
-> 其他电商平台补充
```
...
- `数据角色` 使用目标平台直接信号、搜索趋势信号、内容热度信号或其他平台补充信号。
- 只使用目标市场语言或当地常用表达查询；其他国家的热度不能直接迁移。
- 动态数据必须记录抓取日期；没有日期、入口或可见信号时不得进入热度评分。
- Google Trends 只说明相对搜索趋势，内容平台只说明话题、场景或互动信号，其他电商平台只作补充；均不得表述为目标平台销量或转化率。
- 无法访问时写“未获取”或“受访问限制”，不编造指数、播放量、排名、评论量或增长率，也不把未获取解释为没有需求。
- 热点与商品事实、目标人群或使用场景关联弱时不得采用；高热度不能覆盖合规、授权和真实性门禁。
