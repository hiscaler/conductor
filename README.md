...
如果经常处理相同商品，可以在 [商品资料库](./data/README.md) 中维护 `product-catalog.xlsx`。工作簿包含商品主表、品类扩展属性、规范下拉选项和填写说明，并配合按 `SPU/SKU` 分类的白底图库。以后只提供 SKU、目标平台和目标市场，系统会自动合并商品资料并加载对应主白底图。
...
- 销售单位和组合形态，例如单件、对杯、N 个装、N-pack、套装、混色装、礼盒装，并同步影响标题、主图、包装清单和价格判断。
- 数量写法会自动归一，例如 `3个装`、`3 个装`、`三个装`、`3-pack`、`pack of 3` 都按 3 个装处理；其他数字同理。`N 件套` 这类可能表示多件或配件套装的说法会标记为待确认。
- 无法可靠确认的事实，统一写“待确认”。
