...
核心 Agent 会先展示纯数字启动菜单，让你选择当前业务场景：
...
你输入数字后，Agent 会返回一段带示例值的可复制资料模板。你复制、修改冒号后的内容、提交后，Agent 再开始正式处理；不确定的字段可以留空。

完整启动向导规则见 [启动向导](./workflows/start-guide.md)。
...
如果你通过“你好”启动向导开始，直接改 Agent 返回的示例模板即可。
...
资料不完整也可以开始。核心 Agent 会先整理缺失信息，并标注哪些内容只能作为假设。
...
- 视频字幕默认使用纯文字叠加，不加背景条、色块、气泡、卡片或半透明底。
...
| 菜单    | 出现时机       | 怎么选择                  |
| ----- | ---------- | --------------------- |
| 启动向导  | 刚开始输入“你好”时 | 固定输入数字，例如 `2` 或 `2,7` |
| 下一步动作 | 一次产出完成后    | 优先弹窗选择；没有弹窗时也输入数字     |
...
如果你已经知道要做什么，可以不走“你好”启动向导，直接粘贴完整需求。
...
workflows/
  start-guide.md                     新窗口启动向导和数字流程选择规则
...
docs/
  agent-testing.md                   Agent 规则测试示例和验收清单
...
- 所有平台的卖家侧内容统一使用中文，包括分析、说明、事实、风险、待确认项、验收报告和下一步动作；只有可直接复制到平台、面向买家或用于搜索及广告投放的最终文案值使用目标市场语言。
...
README 面向日常使用，只保留最短路径说明。Agent 规则测试、验收清单和失败排查见 [Agent 测试与验收](./docs/agent-testing.md)。
