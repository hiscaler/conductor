...
如果基本满意，请复制代码块里的商品资料本体，修改不准确或“待确认”的内容后回传给指挥家。当前任务包含文案、图片或视频时，系统会先处理创意方向；存在多个可执行方向时提供 3-5 个方向供用户选择，也可以自动选择；只有一个合理方向时直接形成方案。方向确定后再正式生产。供应链、利润、合规、QA、保存或查看产物等任务不显示创意菜单。
...
单 SKU 且数量为 1 时使用商品资料库中的标准 SKU；数量大于 1 或包含多个 SKU 时自动生成带数量的组合标识；尚未建档的商品使用 `未建档-{简短商品名}`。商品名称和类目记录在 `商品资料.md` 中，不再增加目录层级，因此改名或调整类目不会拆散同一 Listing 的产物。
